    ...
    <div>
      ...
      <div className='flex ml-40 mr-40 justify-between items-center'>
        <p className='text-xl text-slate-500'>
          <span className='text-2xl font-semibold text-slate-500'>Showing 1-12 </span>out of 36 products
        </p>
      ...
      <div className='grid grid-cols-4 gap-5 lg:ml-20 mt-3'>
        {all_product.map((ele, idx) => {
          if (props.category === ele.category) {
            return (
              <Item key={idx} id={ele.id} name={ele.name} image={ele.image} new_price={ele.new_price} old_price={ ele.old_price}/>
            )
          }
          else return null
        })}
